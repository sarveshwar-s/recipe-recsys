...
def home_page():
    ...
    if session["logged_in"]:
        ...
        if user_interaction_count < 10:
            ...
            return render_template("food_list_rein.html", recipe_name=recipe_name, recipe_desc=recipe_desc, recipe_id=recipe_id, recipe_image=recipe_image)
            ...
            if knn_user_id == 4657:
                ...
                return render_template("food_list.html", recipe_name=recipe_name, recipe_image=recipe_image, recipe_desc=recipe_desc, recipe_id=recipe_id_classic)
            else:
                ...
                for index, rows in suggest_list_df.iterrows():
                    prediction_of_recipe_cat.append(svm_loaded_model.predict(uid=knn_user_id, iid= suggest_list_df.loc[index]["RecipeId"]))
                recipe_db_id = []
                ...
                for i in list(predicted_result_list["Images"][:18]):
                    if "['NaN']" in i:
                        return_images_classic.append("https://www.food4fuel.com/wp-content/uploads/woocommerce-placeholder-600x600.png")
                    else:
        ...
        return render_template("food_list_popular.html", recipe_name=pop_recipe_name, recipe_image=recipe_image, recipe_desc=pop_recipe_desc, recipe_id=pop_recipe_id)
    # except:
...
def description(item_number):
    ...
    similar_df = knn_inference(model_load_path= 'models/knn_model.joblib', knn_user_id = user_id, item_id = item_id, output_values= 12)
    ...
    return render_template("food_desc.html", item_number=item_number, recipe_desc_name=recipe_desc_name, 
    recipe_desc_author=recipe_desc_author, recipe_desc_category=recipe_desc_category,
    recipe_desc_review_count = recipe_desc_review_count, recipe_desc_instructions= recipe_desc_instructions,
    recipe_desc_cookTime=recipe_desc_cookTime, recipe_desc_agg_rating=recipe_desc_agg_rating, recipe_desc_dop=recipe_desc_dop,
    sim_names=sim_names, sim_review_count= sim_review_count, sim_ratings=sim_ratings,
    sim_dop = sim_dop, sim_id=sim_id, recipe_image= recipe_desc_img)
...
def description_popular(item_number):
    ...
    return render_template("food_desc_popular.html", item_number=item_number, recipe_desc_name=recipe_desc_name, 
    recipe_desc_author=recipe_desc_author, recipe_desc_category=recipe_desc_category,
    recipe_desc_review_count = recipe_desc_review_count, recipe_desc_instructions= recipe_desc_instructions,
    recipe_desc_cookTime=recipe_desc_cookTime, recipe_desc_agg_rating=recipe_desc_agg_rating, recipe_desc_dop=recipe_desc_dop,
    sim_names=sim_names, sim_review_count= sim_review_count, sim_ratings=sim_ratings,
    sim_dop = sim_dop, sim_id=sim_id, recipe_image=recipe_desc_image)
...
def description_reinforcement(item_number):
    ...
    db_rein = mysql.connector.connect(host=hostname, user=dbusername, passwd=dbpassword,database=database_name)
    rein_sql = "select * from reinforcement_recsys where user_id=" + str(session["user_id"]) + " and recipe_id=" + item_number
    print("****************************************REINFORCEMENT WORKING*****************************************")
    ...
    return render_template("food_desc_reinforcement.html", item_number=item_number, recipe_desc_name=recipe_desc_name, 
    recipe_desc_author=recipe_desc_author, recipe_desc_category=recipe_desc_category,
    recipe_desc_review_count = recipe_desc_review_count, recipe_desc_instructions= recipe_desc_instructions,
    recipe_desc_cookTime=recipe_desc_cookTime, recipe_desc_agg_rating=recipe_desc_agg_rating, recipe_desc_dop=recipe_desc_dop,
    sim_names=sim_names, sim_review_count= sim_review_count, sim_ratings=sim_ratings,
...
def admin_login():
    if request.method == "POST":
        ...
        validation = "select * from admin_users where email='"+request.form["formusername"]+"' AND password='" + request.form["formpassword"] + "'"
...
def login_page():
    if request.method == "POST":
        db=mysql.connector.connect(host=hostname, user=dbusername, passwd=dbpassword,database=database_name)
        logincursor = db.cursor()
        validation = "select * from users where email='"+request.form["formusername"]+"' AND password='" + request.form["formpassword"] + "'"
        logincursor.execute(validation)
        verification = logincursor.fetchall()
        if len(verification)!=0:
            session["key"] = "values" 
...
def recipe_filter_fridge(user_id):
    ...
    return render_template("fridge_results.html", sim_names=sim_names, sim_review_count= sim_review_count, sim_ratings=sim_ratings,
    sim_dop = sim_dop, sim_id=sim_id)
